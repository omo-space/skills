...
description: Produce reviewable level-adjusted word suggestions with validation, safety checks, and structured output. Use when a teacher, tutor, or language learner needs this bounded workflow and will review the result before use.
---

# Vocabulary Enhancer

Produce level-adjusted word suggestions as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or language learner needs level-adjusted word suggestions.
...
3. **Perform:** Produce level-adjusted word suggestions using only the validated request and the server-owned workflow.
