---
name: math-word-problem-explainer
description: Produce reviewable worked explanation with validation, safety checks, and structured output. Use when a teacher, learner, or subject-area tutor needs this bounded workflow and will review the result before use.
---

# Math Word-Problem Explainer

Produce worked explanation as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, learner, or subject-area tutor needs worked explanation.
...
- `wordProblem`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce worked explanation using only the validated request and the server-owned workflow.
4. **Review:** Check subject accuracy and make uncertainty or missing context visible.
...
- Check subject accuracy and make uncertainty or missing context visible.
- Explain the solution process without supplying the final numerical answer.
