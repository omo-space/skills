---
name: vowel-team-finder
description: Produce reviewable marked teams/sound notes with validation, safety checks, and structured output. Use when a teacher, tutor, or literacy specialist needs this bounded workflow and will review the result before use.
---

# Vowel Team Finder

Produce marked teams/sound notes as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or literacy specialist needs marked teams/sound notes.
...
- `listVowelTeamsSeparately`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
- `includeSoundInfo`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce marked teams/sound notes using only the validated request and the server-owned workflow.
