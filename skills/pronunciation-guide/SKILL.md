...
description: Produce reviewable transcription/syllables (no audio today) with validation, safety checks, and structured output. Use when a teacher, tutor, or reading specialist needs this bounded workflow and will review the result before use.
---

# Pronunciation Guide

Produce transcription/syllables (no audio today) as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or reading specialist needs transcription/syllables (no audio today).
...
- `includePhoneticTranscription`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
- `includeSyllableBreakdown`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
- `enableAudioPlayback`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce transcription/syllables (no audio today) using only the validated request and the server-owned workflow.
