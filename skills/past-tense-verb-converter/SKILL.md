---
name: past-tense-verb-converter
description: Produce reviewable form/rule/example with validation, safety checks, and structured output. Use when a teacher, tutor, or language learner needs this bounded workflow and will review the result before use.
---

# Past-Tense Verb Converter

Produce form/rule/example as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or language learner needs form/rule/example.
...
- `baseVerb`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `includeRules`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce form/rule/example using only the validated request and the server-owned workflow.
