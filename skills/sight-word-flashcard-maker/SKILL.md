---
name: sight-word-flashcard-maker
description: Produce reviewable card-copy plan with validation, safety checks, and structured output. Use when a teacher or resource author needs this bounded workflow and will review the result before use.
---

# Sight-Word Flashcard Maker

Produce card-copy plan as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher or resource author needs card-copy plan.
...
- `sightWords`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `cardStyle`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
- `fontSize`: observed source payload field; proposed target constraint is integer; range 1-100. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce card-copy plan using only the validated request and the server-owned workflow.
