...
description: Produce reviewable audience-level explanation with validation, safety checks, and structured output. Use when a teacher, learner, or subject-area tutor needs this bounded workflow and will review the result before use.
---

# Science-Concept Explainer

Produce audience-level explanation as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, learner, or subject-area tutor needs audience-level explanation.
...
- `conceptName`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `audienceLevel`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce audience-level explanation using only the validated request and the server-owned workflow.
