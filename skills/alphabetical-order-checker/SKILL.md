---
name: alphabetical-order-checker
description: Produce reviewable verdict/correct order with validation, safety checks, and structured output. Use when a teacher, tutor, or language learner needs this bounded workflow and will review the result before use.
---

# Alphabetical Order Checker

Produce verdict/correct order as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or language learner needs verdict/correct order.
...
- `caseSensitive`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
- `includeCorrectedList`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce verdict/correct order using only the validated request and the server-owned workflow.
