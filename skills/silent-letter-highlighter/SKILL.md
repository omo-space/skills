---
name: silent-letter-highlighter
description: Produce reviewable marked text/word list with validation, safety checks, and structured output. Use when a teacher, tutor, or literacy specialist needs this bounded workflow and will review the result before use.
---

# Silent-Letter Highlighter

Produce marked text/word list as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or literacy specialist needs marked text/word list.
...
- `includeSilentLetter`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce marked text/word list using only the validated request and the server-owned workflow.
