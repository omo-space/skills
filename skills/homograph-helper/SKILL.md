---
name: homograph-helper
description: Produce reviewable meanings/pronunciations/examples with validation, safety checks, and structured output. Use when a teacher, tutor, or language learner needs this bounded workflow and will review the result before use.
---

# Homograph Helper

Produce meanings/pronunciations/examples as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or language learner needs meanings/pronunciations/examples.
...
- `includeMeanings`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
- `includePronunciations`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
- `includeExampleSentences`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce meanings/pronunciations/examples using only the validated request and the server-owned workflow.
