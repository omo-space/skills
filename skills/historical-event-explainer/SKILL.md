...
description: Produce reviewable leveled overview with validation, safety checks, and structured output. Use when a teacher, learner, or subject-area tutor needs this bounded workflow and will review the result before use.
---

# Historical-Event Explainer

Produce leveled overview as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, learner, or subject-area tutor needs leveled overview.
...
- `eventName`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `detailLevel`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce leveled overview using only the validated request and the server-owned workflow.
