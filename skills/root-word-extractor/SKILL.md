---
name: root-word-extractor
description: Produce reviewable root/affix analysis with validation, safety checks, and structured output. Use when a teacher, tutor, or language learner needs this bounded workflow and will review the result before use.
---

# Root Word Extractor

Produce root/affix analysis as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or language learner needs root/affix analysis.
...
- `includeAffixes`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
- `highlightRoot`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce root/affix analysis using only the validated request and the server-owned workflow.
