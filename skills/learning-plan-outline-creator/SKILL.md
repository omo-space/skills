---
name: learning-plan-outline-creator
description: Produce reviewable sequenced plan with validation, safety checks, and structured output. Use when a teacher or instructional lead needs this bounded workflow and will review the result before use.
---

# Learning-Plan Outline Creator

Produce sequenced plan as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher or instructional lead needs sequenced plan.
...
- `subject`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `duration`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `learningGoal`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce sequenced plan using only the validated request and the server-owned workflow.
