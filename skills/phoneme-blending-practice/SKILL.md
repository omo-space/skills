---
name: phoneme-blending-practice
description: Produce reviewable segmented/revealed words with validation, safety checks, and structured output. Use when a teacher, tutor, or literacy specialist needs this bounded workflow and will review the result before use.
---

# Phoneme Blending Practice

Produce segmented/revealed words as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or literacy specialist needs segmented/revealed words.
...
- `numPhonemes`: observed source payload field; proposed target constraint is integer; range 1-100. Confirm requiredness, default, and allowed values against the source before activation.
...
- `includeWordReveal`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce segmented/revealed words using only the validated request and the server-owned workflow.
