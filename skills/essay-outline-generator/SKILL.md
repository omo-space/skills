...
description: Produce reviewable paragraph outline with validation, safety checks, and structured output. Use when a teacher, student, or literacy-resource author needs this bounded workflow and will review the result before use.
---

# Essay Outline Generator

Produce paragraph outline as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, student, or literacy-resource author needs paragraph outline.
...
- `essayTopic`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `essayType`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
- `numParagraphs`: observed source payload field; proposed target constraint is integer; range 1-100. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce paragraph outline using only the validated request and the server-owned workflow.
