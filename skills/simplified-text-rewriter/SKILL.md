---
name: simplified-text-rewriter
description: Produce reviewable age-adjusted rewrite with validation, safety checks, and structured output. Use when a teacher, tutor, or language learner needs this bounded workflow and will review the result before use.
---

# Simplified Text Rewriter

Produce age-adjusted rewrite as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or language learner needs age-adjusted rewrite.
...
- `targetAudienceAge`: observed source payload field; proposed target constraint is integer; range 1-100. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce age-adjusted rewrite using only the validated request and the server-owned workflow.
