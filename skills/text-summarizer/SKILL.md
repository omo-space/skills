...
description: Produce reviewable bounded summary with validation, safety checks, and structured output. Use when a teacher, student, or literacy-resource author needs this bounded workflow and will review the result before use.
---

# Text Summarizer

Produce bounded summary as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, student, or literacy-resource author needs bounded summary.
...
- `textToSummarize`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `summaryLength`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
- `focusKeywords`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce bounded summary using only the validated request and the server-owned workflow.
