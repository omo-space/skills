---
name: minimal-pairs-generator
description: Produce reviewable contrast pairs with validation, safety checks, and structured output. Use when a teacher, tutor, or literacy specialist needs this bounded workflow and will review the result before use.
---

# Minimal Pairs Generator

Produce contrast pairs as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or literacy specialist needs contrast pairs.
...
- `focusDescription`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
...
- `positionOfChange`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce contrast pairs using only the validated request and the server-owned workflow.
