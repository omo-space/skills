---
name: high-frequency-word-checker
description: Produce reviewable marked words/coverage with validation, safety checks, and structured output. Use when a teacher, tutor, or literacy specialist needs this bounded workflow and will review the result before use.
---

# High-Frequency Word Checker

Produce marked words/coverage as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or literacy specialist needs marked words/coverage.
...
- `frequencyList`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce marked words/coverage using only the validated request and the server-owned workflow.
