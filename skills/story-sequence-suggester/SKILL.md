---
name: story-sequence-suggester
description: Produce reviewable ordered events/reasoning with validation, safety checks, and structured output. Use when a teacher, tutor, or reading specialist needs this bounded workflow and will review the result before use.
---

# Story Sequence Suggester

Produce ordered events/reasoning as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or reading specialist needs ordered events/reasoning.
...
- `eventsInput`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `sequenceType`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
- `includeReasoning`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce ordered events/reasoning using only the validated request and the server-owned workflow.
