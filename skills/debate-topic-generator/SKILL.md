...
description: Produce reviewable age/complexity-adjusted topics with validation, safety checks, and structured output. Use when a teacher, student, or literacy-resource author needs this bounded workflow and will review the result before use.
---

# Debate Topic Generator

Produce age/complexity-adjusted topics as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, student, or literacy-resource author needs age/complexity-adjusted topics.
...
- `subjectArea`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `complexity`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
- `numTopics`: observed source payload field; proposed target constraint is integer; range 1-100. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce age/complexity-adjusted topics using only the validated request and the server-owned workflow.
