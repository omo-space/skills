---
name: phoneme-segmentation-practice
description: Produce reviewable word/phoneme reveal set with validation, safety checks, and structured output. Use when a teacher, tutor, or literacy specialist needs this bounded workflow and will review the result before use.
---

# Phoneme Segmentation Practice

Produce word/phoneme reveal set as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher, tutor, or literacy specialist needs word/phoneme reveal set.
...
- `numPhonemesTarget`: observed source payload field; proposed target constraint is integer; range 1-100. Confirm requiredness, default, and allowed values against the source before activation.
...
- `includePhonemeReveal`: observed source payload field; proposed target constraint is boolean. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce word/phoneme reveal set using only the validated request and the server-owned workflow.
4. **Review:** Check phoneme, grapheme, syllable, dialect, and age-level accuracy.
