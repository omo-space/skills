...
description: Produce reviewable questions/options/key with validation, safety checks, and structured output. Use when a teacher or resource author needs this bounded workflow and will review the result before use.
---

# Multiple-Choice Quiz Generator

Produce questions/options/key as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher or resource author needs questions/options/key.
...
- `topic`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
...
- `numQuestions`: observed source payload field; proposed target constraint is integer; range 1-100. Confirm requiredness, default, and allowed values against the source before activation.
- `difficulty`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce questions/options/key using only the validated request and the server-owned workflow.
