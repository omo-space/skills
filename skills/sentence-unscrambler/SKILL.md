...
description: Produce reviewable scrambled prompt/answer with validation, safety checks, and structured output. Use when a teacher or resource author needs this bounded workflow and will review the result before use.
---

# Sentence Unscrambler

Produce scrambled prompt/answer as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher or resource author needs scrambled prompt/answer.
...
- `sentence`: observed source payload field; proposed target constraint is string. Confirm requiredness, default, and allowed values against the source before activation.
- `scrambleUnits`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce scrambled prompt/answer using only the validated request and the server-owned workflow.
