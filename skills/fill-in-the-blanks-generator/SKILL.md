...
description: Produce reviewable exercise/key with validation, safety checks, and structured output. Use when a teacher or resource author needs this bounded workflow and will review the result before use.
---

# Fill-In-The-Blanks Generator

Produce exercise/key as a bounded workflow contract derived from the inspected
PhonicsMaker behavior, without claiming deployment or classroom approval.

## When to use

- A teacher or resource author needs exercise/key.
...
- `blankType`: observed source payload field; proposed target constraint is string; allowed source values still require review before activation. Confirm requiredness, default, and allowed values against the source before activation.
...
3. **Perform:** Produce exercise/key using only the validated request and the server-owned workflow.
